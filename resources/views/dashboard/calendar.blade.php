                    ...
                    <div class="panel-body">
                        ...
                        <table class="calendar">
                            ...
                            @for($i = 1; $i < $month_days; $i++)
                                ...
                                @endphp<td class="day" id="dayEvent" style="background-color:mediumseagreen;"><a href="/date/show/{{$availables}}/{{$date}}">{{$i}}<br /></a></td>@php
                                    $show = true;
                                   }
                                   elseif($availableDate == $date AND $c == 2 AND $v == $availables){
                                @endphp<td class="day" id="dayEvent" style="background-color:yellow;"><a href="/date/show/{{$availables}}/{{$date}}">{{$i}}<br /></a><br /></td>@php
                                    $show = true;
                                  }
                                  elseif($availableDate == $date AND $c == NULL AND $v == $availables){
                                @endphp<td class="day" id="dayEvent" style="background-color:indianred;"><a href="/date/show/{{$availables}}/{{$date}}">{{$i}}<br /></a><br /></td>@php
                                    $show = true;
                                  }
                                  }
                               }
                            if($show == true){}
                                ...
                                @endphp<td class="day" id="dayEvent"><a href="/date/create/{{$i}}-{{$month}}-{{$year}}">{{$i}}<br /></a><br /></td><?php
                                }?>
                                @if(($i + $blank) % 7 == 0)
                                </tr><tr>
                                    @else
